...
async function save() {
  ...
  radioGroups.forEach((group) => {
    ...
    radios.forEach((radio) => {
      if (!isChecked) {
        ...
        } else if (radio.name === "cig") {
          console.log("not selected cig");
          label = document.getElementById("cig_label");
          label.setAttribute("style", "color: red;");
        } else if (radio.name === "alc") {
          label = document.getElementById("alc");
          label.setAttribute("style", "color: red;");
        }
        radio.closest("div").style.borderColor = "red";
      } else {
        radio.closest("div").style.borderColor = "initial";
      }
    });
  });
  ...
  if (illness_button === "yes") {
    ...
    const checkboxGroups = Array.from(
      new Set(checkboxes.map((checkbox) => checkbox.id))
    );
